...
const MySchedule = () => {
  ...
  const fetchData = async () => {
    try {
      await instance.get("/api/schedule/list").then((res) => setTemp(res.data));
    } catch (error) {
      console.log(error);
    }
  };
          ...
          >
            ...
            <label className="flex">
              ...
              <div className=" px-12 flex flex-col justify-center items-center">
                ...
                <p>{item.tdsEntity.tsTpEntity.tsEntity.tsName}</p>
              </div>
              <div className="flex-1 flex flex-col relative justify-center gap-2">
                ...
                <p className="font-bold">
                  여행일자
                  <span className="font-normal ml-4 text-sm">
                    {item.tdsEntity.tsTpEntity.tsEntity.tsStartDate}~
                    {item.tdsEntity.tsTpEntity.tsEntity.tsEndDate}
                  </span>
                </p>
                {/* 모달 */}
